...
def is_thai_codepoint(cp: int) -> bool:
    return 0x0E01 <= cp <= 0x0E5B


def is_thai_only(codepoints: list[int]) -> bool:
    return bool(codepoints) and all(is_thai_codepoint(cp) for cp in codepoints)
...
def generate(
    ...
    for line in input_path.read_text(encoding="utf-8").splitlines():
        ...
        if not is_thai_only(codepoints):
            continue
