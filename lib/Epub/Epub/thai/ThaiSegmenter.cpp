...
inline bool isThaiLeadingVowel(uint32_t cp) { return cp >= 0x0E40 && cp <= 0x0E44; }
...
// combining marks (vowels, tone marks). A leading vowel is grouped with the
// following consonant cluster, with an orphan combining mark allowed to start
// a cluster when the source is malformed or already decomposed.
std::vector<ThaiCluster> parseClusters(const std::string& text, size_t start, size_t end) {
  ...
  while (ptr < endPtr) {
    ...
    if (isThaiLeadingVowel(cp) && ptr < endPtr) {
    ...
    // Sara Am is decomposed for rendering as Nikhahit (U+0E4D) followed by
    // Sara Aa (U+0E32). Keep that expansion atomic even though Sara Aa is a
    // spacing vowel rather than a combining mark. This also handles explicit
    // U+0E4D/U+0E32 source text and tone marks between the pair.
    bool hasNikhahit = cp == 0x0E4D;
    ...
    while (ptr < endPtr) {
      ...
      if (nextCp != 0 && utf8IsCombiningMark(nextCp)) {
        hasNikhahit = hasNikhahit || nextCp == 0x0E4D;
        continue;
      }
      if (nextCp == 0x0E32 && hasNikhahit) {
        continue;
      }
      ptr = beforeCp;
      break;
