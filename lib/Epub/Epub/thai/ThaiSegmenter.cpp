...
#include "ThaiDict.h"
#include "generated/thaiDict.generated.h"
...
// exceed the lookup buffer and are skipped by thaiDictContains anyway.
constexpr size_t MAX_WORD_CLUSTERS = 20;

// Stack buffer size for dictionary lookup in thaiDictContains().
// Must match the limit in ThaiDict.h.
constexpr size_t MAX_LOOKUP_BYTES = 128;

const ThaiDict& getThaiDict() { return thai_dict; }
...
// Segment a Thai run using forward maximal matching against the dictionary.
...
void segmentThaiRun(const std::string& text, size_t start, size_t end, const ThaiDict& dict,
  ...
  while (pos < clusters.size()) {
    // Default: single cluster (no dictionary match). This means unknown words
    ...
    for (size_t len = maxLen; len >= 2; --len) {
      ...
      if (thaiDictContains(dict, text.c_str() + substrStart, substrLen)) {
        bestEnd = pos + len - 1;
        break;
      }
    }
...
std::vector<size_t> thaiWordBreakByteOffsets(const std::string& text) {
  ...
  while (*ptr) {
    ...
    if (isThaiCodepoint(cp)) {
      ...
      segmentThaiRun(text, runStartOffset, runEndOffset, getThaiDict(), breaks);
