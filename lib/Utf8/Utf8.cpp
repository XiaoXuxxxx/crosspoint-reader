...
size_t utf8LastCodepointStart(const char* buf, const size_t len) {
  if (len == 0) return 0;
  size_t pos = len - 1;
...
std::string utf8DecomposeThaiSaraAm(const std::string& in) {
  ...
  // This matches gen_thai_lexicon.py decompose_sara_am() exactly.
  //
  ...
  while (*p) {
    ...
    if (cp == 0x0E33) {
      ...
      std::string tones;
      while (!out.empty()) {
        const size_t pos = utf8LastCodepointStart(out.data(), out.size());
...
size_t utf8RemoveLastChar(std::string& str) {
  ...
  const size_t pos = utf8LastCodepointStart(str.data(), str.size());
  str.resize(pos);
  return pos;
}
